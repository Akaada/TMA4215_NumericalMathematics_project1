...
def chebyshev_nodes(a, b, n):
    ...
    t = np.cos(np.arange(n + 1) * np.pi / n)[::-1]   # ascending order
    return 0.5 * (b - a) * t + 0.5 * (b + a)         # Psi: [-1, 1] -> [a, b]
...
def plot_convergence(functions, n0, n_end, n_step=1,nodes = ["equidistant", "chebyshev"], N=1000):
    ...
    functions : list of tuples
        Tuples of the form (f, a, b).
    nodes : list of str
        Node types, for example ["equidistant", "chebyshev"].
    """
    n_vals = np.arange(n0, n_end + 1, n_step)
...
if __name__ == "__main__":
    ...
    # a) Runge function, equidistant vs Chebyshev, n = 10
    g_eq = make_lagrange(runge, a, b, 10, "equidistant")
    g_ch = make_lagrange(runge, a, b, 10, "chebyshev")
    plot_interpolant(a, b, g_eq, g_ch, title="Runge function, n = 10")

    # b) error decay for cos(2*pi*x) on [0, 1]
    ns = np.arange(2, 21)
    res = {
        "equidistant": convergence(f_cos, 0, 1, ns,
                                   lambda n: make_lagrange(f_cos, 0, 1, n)),
        "Chebyshev": convergence(f_cos, 0, 1, ns,
                                 lambda n: make_lagrange(f_cos, 0, 1, n,
                                                         "chebyshev")),
    }
    plot_convergence(res, xlabel="n", title="cos(2*pi*x) on [0, 1]")

    # c) piecewise interpolation, error as a function of K
    Ks = np.array([2, 4, 8, 16, 32, 64])
    res_pw = {"piecewise n = 3":
              convergence(runge, a, b, Ks,
                          lambda K: make_piecewise(runge, a, b, K, 3))}
    plot_convergence(res_pw, xlabel="K", title="Piecewise interpolation, Runge")